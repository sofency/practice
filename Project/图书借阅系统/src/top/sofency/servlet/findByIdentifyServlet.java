 ...
 * @Version: 1.0
 */
@WebServlet(name = "findByIdentifyServlet")
public class findByIdentifyServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String identify=request.getParameter("identify");//获取作者的姓名
        loginFunction func=new loginFunction();//创建对象
        ArrayList<book> list=null;//创建列表
        try {
            list = func.searchByType(identify);
